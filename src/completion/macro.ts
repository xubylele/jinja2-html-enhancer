...
export class MacroSignatureHelpProvider implements vscode.SignatureHelpProvider {
  ...
  async provideSignatureHelp(
    ...
    const resolved = await this.resolveCrossFileMacro(document.uri, call.macroName, call.namespace);
    if (!resolved) {
      return null;
    }
  ...
  private buildHelp(
    ...
    const signature = new vscode.SignatureInformation(label, new vscode.MarkdownString(docLabel));
