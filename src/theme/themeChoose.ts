...
export const chooseThemeSelector = () => {
  ...
  return vscode.window
    ...
    .then(async (selection) => {
      ...
      const newRules = jinja2Colors.textMateRules.filter((rule) => !existingScopes.has(rule.scope));
      const merged = {
        ...currentTheme,
        textMateRules: [
          ...(Array.isArray((currentTheme as any)?.textMateRules)
            ? (currentTheme as any).textMateRules
            : []),
          ...newRules,
        ],
      };
      ...
      try {
        await vscode.workspace
          .getConfiguration()
          .update("editor.tokenColorCustomizations", merged, target);
