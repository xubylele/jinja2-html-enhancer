...
describe('MacroCompletionProvider', () => {
  ...
  it('returns null outside print context', () => {
    ...
    expect(provider.provideCompletionItems(doc, { line: 0, character: 8 } as any))
      .toBeNull();
  });
...
describe('MacroSignatureHelpProvider', () => {
  ...
  it('returns null when not inside a call', () => {
    ...
    expect(provider.provideSignatureHelp(doc, { line: 0, character: 7 } as any))
      .toBeNull();
  });
  ...
  it('returns null for namespaced calls (Pro territory)', () => {
    ...
    const doc = makeDocument(TEMPLATE, line, line.length);
    expect(provider.provideSignatureHelp(doc, { line: 0, character: line.length } as any))
      .toBeNull();
  });
  ...
  it('builds a signature for a known local macro', () => {
    const line = '{{ card(';
    const doc = makeDocument(TEMPLATE, line, line.length);
    const help = provider.provideSignatureHelp(doc, { line: 0, character: line.length } as any) as any;
